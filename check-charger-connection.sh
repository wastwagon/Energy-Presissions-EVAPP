...
# Check OCPP Gateway logs for recent connections
echo "Recent connection attempts (last 5 minutes):"
docker-compose logs --since 5m ocpp-gateway 2>/dev/null | grep -E "connection|WebSocket|charge point|BootNotification" | tail -10

if [ $? -ne 0 ] || [ -z "$(docker-compose logs --since 5m ocpp-gateway 2>/dev/null | grep -E 'connection|WebSocket|charge point')" ]; then
